...
class App extends Component {
  state = {check: tabsList[0].tabId}

  stateChangedFunction = tabId => {
    this.setState({check: tabId})
  }

  render() {
    const {check} = this.state
    const filteredList = projectsList.filter(each => each.category === check)
    return (
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              stateChangedFunction={this.stateChangedFunction}
            />
          ))}
        </ul>
